...
const requireAllTestFiles = (pathToSearch, specification, fileType) => {
  readDir(pathToSearch).forEach(file => {
    if (!isDirectory(`${pathToSearch}/${file}`)) {
      ...
      requireAllTestFiles(`${pathToSearch}/${file}`, specification, fileType);
    }
  });
...
exports.generateDocumentationObject = options => {
  ...
  return requireAllTestFiles(normalizedPath, specification, options.fileType);
};
